...
def parsePoint(line):
    ...
    return LabeledPoint(lineFeatures[0], lineFeatures[1:])
...
onlyLabels = parsedDataInit.map(lambda labeledPoint : labeledPoint.label)
...
def calcRMSE(labelsAndPreds):
    ...
    """
    return np.sqrt(labelsAndPreds.map(lambda (label,prediction) : squaredError(label,prediction)).mean())
...
def twoWayInteractions(lp):
    ...
    return LabeledPoint(lp.label, 
                        np.hstack((
                            lp.features, 
                            [t[0]*t[1] for t in itertools.product(lp.features,repeat=2)]))
                       )
