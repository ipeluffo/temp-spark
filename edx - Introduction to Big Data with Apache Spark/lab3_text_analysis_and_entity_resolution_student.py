...
def simpleTokenize(string):
    ...
    """
    tokens = re.split(split_regex, string.strip().lower())
...
def tokenize(string):
    ...
    """
    return [token for token in re.split(split_regex, string.strip().lower()) if token and (token not in stopwords)]
...
# TODO: Replace <FILL IN> with appropriate code
amazonRecToToken = amazonSmall.map(lambda (id, prod) : (id,tokenize(prod)))
googleRecToToken = googleSmall.map(lambda (id, prod) : (id,tokenize(prod)))
...
def findBiggestRecord(vendorRDD):
    ...
    """
    return (vendorRDD.takeOrdered(1, lambda (id, tokens) : -len(tokens)))
...
def idfs(corpus):
    ...
    """
    N = corpus.count()
    uniqueTokens = corpus.flatMap(lambda (key, tokens) : list(set(tokens)))
    ...
    return (tokenSumPairTuple.map(lambda (token, count) : (token, N / float(count))))
...
def dotprod(a, b):
    ...
    """
    return sum([a[token] * b[token] for token in a if a.has_key(token) * b.has_key(token)])
...
def norm(a):
    ...
    """
    return math.sqrt(sum([math.pow(a[token],2) for token in a]))
...
avgSimDups = trueDupsRDD.sum() / float(trueDupsCount)

nonDupsRDD = (sims
              .leftOuterJoin(goldStandard)
              .filter(lambda record : record[1][1] == None)
              .map(lambda record : record[1][0]))
avgSimNon = nonDupsRDD.sum() / float(nonDupsRDD.count())
...
# TODO: Replace <FILL IN> with appropriate code
amazonFullRecToToken = amazon.map(lambda (id, value) : (id, tokenize(value)))
googleFullRecToToken = google.map(lambda (id, value) : (id, tokenize(value)))
...
# TODO: Replace <FILL IN> with appropriate code
amazonNorms = amazonWeightsRDD.map(lambda (id, weights) : (id, math.sqrt(sum([math.pow(weights[token],2) for token in weights]))))
amazonNormsBroadcast = sc.broadcast(amazonNorms.collectAsMap())
googleNorms = googleWeightsRDD.map(lambda (id, weights) : (id, math.sqrt(sum([math.pow(weights[token],2) for token in weights]))))
...
def fastCosineSimilarity(record):
    ...
    value = s / float(norm(amazonWeightsBroadcast.value[amazonRec]) * norm(googleWeightsBroadcast.value[googleRec]))
