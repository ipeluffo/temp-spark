...
def getCountsAndAverages(IDandRatingsTuple):
    ...
    """
    number_rating = len(IDandRatingsTuple[1])
    average_rating = sum(IDandRatingsTuple[1]) / float(number_rating)
    return (IDandRatingsTuple[0], (number_rating, average_rating))
...
movieNameWithAvgRatingsRDD = (moviesRDD
                              ...
                              .map(lambda t : (t[1][1][1], t[1][0], t[1][1][0])))
...
def computeError(predictedRDD, actualRDD):
    ...
    squaredErrorsRDD = (predictedReformattedRDD
                        ...
                        .map(lambda t : math.pow(t[1][0]-t[1][1],2)))
...
# TODO: Replace <FILL IN> with appropriate code
myModel = ALS.train(trainingRDD, 8, iterations=5, seed=5L, lambda_=0.1)
...
trainingAvgRating = trainingRDD.map(lambda t : t[2]).sum() / float(trainingRDD.count())
...
myUnratedMoviesRDD = (moviesRDD
                      .filter(lambda t : t[0] not in [t[1] for t in myRatedMovies])
                      .map(lambda t : (0, t[0])))
...
ratingsWithNamesRDD = (predictedWithCountsRDD
                       .join(moviesRDD)
                       .map(lambda t : (t[1][0][0], t[1][1], t[1][0][1]))
                       .filter(lambda t : t[2] > 75))
